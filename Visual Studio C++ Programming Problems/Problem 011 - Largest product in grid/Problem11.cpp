...
int main() {
	...
	cout << "The Largest product in grid is " << findGreatestProd(Array,20) << endl;
...
int findVert(int** Arr, int len) {
	...
	for (int j = 0; j < len; j++) {
		for (int i = 0; i < len - 4 + 1; i++) {
			Prod = Arr[i + 0][j] * Arr[i + 1][j] * Arr[i + 2][j] * Arr[i + 3][j];
			if (Prod > maxProd)
				maxProd = Prod;
		}
	}
...
int findHoriz(int** Arr, int len) {
	int maxProd = 1;
	int Prod = 1;
	for (int i = 0; i < len; i++) {
		for (int j = 0; j < len - 4 + 1; j++) {
			Prod = Arr[i][j + 0] * Arr[i][j + 1] * Arr[i][j + 2] * Arr[i][j + 3];
			if (Prod > maxProd)
				maxProd = Prod;
		}
	}
